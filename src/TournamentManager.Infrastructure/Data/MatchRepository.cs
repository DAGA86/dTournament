...
{
    public async Task<IReadOnlyList<Match>> ListByAgeGroupAsync(Guid ageGroupId, CancellationToken cancellationToken = default) => await dbContext.Matches
        .Include(x => x.HomeTeam)
        .Include(x => x.AwayTeam)
        .Include(x => x.Venue)
        .Include(x => x.Group)
        .Include(x => x.GoalEvents).ThenInclude(x => x.Team)
        .Include(x => x.GoalEvents).ThenInclude(x => x.Player)
