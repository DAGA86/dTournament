...
{
    public async Task<IReadOnlyList<Match>> ListByAgeGroupAsync(Guid ageGroupId, CancellationToken cancellationToken = default) => await dbContext.Matches.Include(x => x.HomeTeam).Include(x => x.AwayTeam).Include(x => x.Venue).Include(x => x.Group).Include(x => x.GoalEvents).ThenInclude(x => x.Team).Include(x => x.GoalEvents).ThenInclude(x => x.Player).Where(x => x.AgeGroupId == ageGroupId).OrderBy(x => x.RoundNumber).ThenBy(x => x.ScheduledStartUtc).ThenBy(x => x.Group == null ? int.MaxValue : x.Group.DisplayOrder).ThenBy(x => x.Group == null ? string.Empty : x.Group.Name).ToListAsync(cancellationToken);
