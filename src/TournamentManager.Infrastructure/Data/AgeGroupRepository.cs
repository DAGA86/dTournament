...
public sealed class AgeGroupRepository(ApplicationDbContext dbContext) : IAgeGroupRepository
{
    ...
    public async Task<IReadOnlyList<AgeGroup>> ListByTournamentAsync(Guid tournamentId, CancellationToken cancellationToken = default) => await dbContext.AgeGroups.Where(x => x.TournamentId == tournamentId).OrderBy(x => x.DisplayOrder).ThenBy(x => x.Name).ToListAsync(cancellationToken);
