...
public sealed class PlayerOfTheMatchVoteConfiguration : IEntityTypeConfiguration<PlayerOfTheMatchVote>
{
    public void Configure(EntityTypeBuilder<PlayerOfTheMatchVote> builder)
    {
        ...
        builder.Property(x => x.IsGoalkeeperVote).HasDefaultValue(false);
        ...
        builder.HasIndex(x => new { x.MatchId, x.TeamId }).IsUnique().HasFilter("[IsGoalkeeperVote] = 0");
        builder.HasIndex(x => x.MatchId).IsUnique().HasFilter("[IsGoalkeeperVote] = 1");
    }
}
