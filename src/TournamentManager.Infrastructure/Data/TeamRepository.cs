...
{
    public async Task<IReadOnlyList<Team>> ListByAgeGroupAsync(Guid ageGroupId, CancellationToken cancellationToken = default) => await dbContext.Teams.Include(x => x.AgeGroup).Include(x => x.Group).Include(x => x.Players).Where(x => x.AgeGroupId == ageGroupId).OrderBy(x => x.Name).ToListAsync(cancellationToken);
    public Task<Team?> GetAsync(Guid id, CancellationToken cancellationToken = default) => dbContext.Teams.Include(x => x.AgeGroup).Include(x => x.Group).Include(x => x.Players).FirstOrDefaultAsync(x => x.Id == id, cancellationToken);
